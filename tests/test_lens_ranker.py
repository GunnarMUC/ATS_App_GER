...
def test_plan_orders_differ_ceo_vs_coo(fixtures_dir: Path):
    ...
    assert set(coo["hidden_experience_ids"]).issubset(
        {e["id"] for e in facts["experience"]}
    )
    # skill emphasis differs
    assert coo["skill_order"][0] != ceo["skill_order"][0] or set(
        coo["emphasis_kpi_ids"]
    ) != set(ceo["emphasis_kpi_ids"]) or set(coo["emphasis_bullet_ids"]) != set(
        ceo["emphasis_bullet_ids"]
    )
